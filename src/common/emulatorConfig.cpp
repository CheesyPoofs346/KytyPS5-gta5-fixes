...
static std::atomic<bool> g_pipeline_names {false};
...
void Load(const ConfigOptions& cfg) {
	...
	*g_config = cfg;
	...
	g_pipeline_names.store(cfg.pipeline_names, std::memory_order_relaxed);
...
bool PipelineNamesEnabled() {
	return g_pipeline_names.load(std::memory_order_relaxed);
}
...
void LogEffectiveSettings() {
	...
	std::printf("  %-28s %s\n", "pipeline_names", g_pipeline_names.load(std::memory_order_relaxed) ? "true" : "false");
