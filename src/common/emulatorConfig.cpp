...
bool GpuTimestampsEnabled() {
	return g_config->gpu_timestamps;
}

const std::vector<uint64_t>& GpuTimestampsPsChksums() {
	return g_config->gpu_timestamps_ps_chksum;
}
...
void LogEffectiveSettings() {
	...
	std::printf("  %-28s %s\n", "gpu_timestamps", g_config->gpu_timestamps ? "true" : "false");
	std::printf("  %-28s %zu\n", "gpu_timestamps_ps_chksum", g_config->gpu_timestamps_ps_chksum.size());
