...
// Test builds only: counts condition wait returns and notified returns that found no wake.
static std::atomic<uint64_t> g_cond_test_wait_returns {0};
static std::atomic<uint64_t> g_cond_test_notified_not_ready {0};

PthreadCondWakeCounts PthreadCondTakeWakeCounts() {
	return {g_cond_test_wait_returns.exchange(0, std::memory_order_relaxed),
	        g_cond_test_notified_not_ready.exchange(0, std::memory_order_relaxed)};
}
#endif

template <typename Duration, typename Ready>
static void CondWaitForCounted(std::condition_variable& cv, std::unique_lock<std::mutex>& lock,
                               Duration duration, const Ready& ready) {
#if defined(KYTY_PTHREAD_COND_WAKE_COUNT)
	const auto status = cv.wait_for(lock, duration);
	g_cond_test_wait_returns.fetch_add(1, std::memory_order_relaxed);
	if (status == std::cv_status::no_timeout && !ready()) {
		g_cond_test_notified_not_ready.fetch_add(1, std::memory_order_relaxed);
	}
#else
	(void)ready;
	...
	if (usec == 0) {
		...
		while (!ready()) {
			...
			const auto poll      = (remaining < std::chrono::microseconds(SIGNAL_APC_POLL_MICROS)
			                            ...
			                            : std::chrono::steady_clock::duration(
			                                  std::chrono::microseconds(SIGNAL_APC_POLL_MICROS)));
			CondWaitForCounted(thread->cond_cv, cond_lock, poll, ready);
...
int KYTY_SYSV_ABI PthreadCondWait(PthreadCond* cond, PthreadMutex* mutex) {
	...
	while (!ready()) {
		CondWaitForCounted(thread->cond_cv, cond_lock, std::chrono::microseconds(SIGNAL_APC_POLL_MICROS), ready);
		if (!ready()) {
			cond_lock.unlock();
			KernelDispatchPendingSignalForCurrentThread();
			cond_lock.lock();
