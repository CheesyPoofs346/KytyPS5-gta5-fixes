...
// Narrow access shim for the real CommitBindings path. The production executor deliberately
// owns descriptor-pipeline construction; this test only supplies the otherwise-private minimal
// pipeline needed to make its push-descriptor call observable by validation.
namespace Libs::Graphics {
struct RenderExecutorTestAccess {
	static PipelineCache::Pipeline CommitSingleStage(RenderExecutor& executor, CommandBuffer& buffer,
	                                                PreparedBindings& bindings) {
		const auto& program = *bindings.program;
		std::vector<vk::DescriptorSetLayoutBinding> layout_bindings;
		for (const auto& binding: program.bindings.descriptors) {
			layout_bindings.push_back({ShaderRecompiler::IR::NativeBinding(program.stage, binding.kind),
		...
		descriptor_info.bindingCount = static_cast<uint32_t>(layout_bindings.size());
		...
		EXIT_IF(device.createDescriptorSetLayout(&descriptor_info, nullptr,
		                                         &pipeline.descriptor_set_layout) != vk::Result::eSuccess);
		...
		EXIT_IF(device.createPipelineLayout(&pipeline_info, nullptr, &pipeline.pipeline_layout) !=
		        vk::Result::eSuccess);
		pipeline.uses_push_descriptors = true;
		std::array<PreparedBindings*, 1> stages{&bindings};
		executor.CommitBindings(buffer, vk::PipelineBindPoint::eCompute, pipeline, stages);
		return pipeline;
	}

	static void Destroy(RenderExecutor& executor, const PipelineCache::Pipeline& pipeline) {
		auto& device = executor.m_context.GetGraphics().device;
...
int main() {
	...
	// above only established that the lookup primitive returns the right thing.
	{
		...
		// This fixture supplies two hand-built native storage views. Declare the matching
		// binding exactly as production allocation does; the old all-slot loop happened to
		// tolerate its absence, while filtered production code correctly does not.
		program.bindings.descriptors.push_back(
		    {ShaderRecompiler::IR::DescriptorBindingKind::Buffers, {0u, 1u}});
		...
		stage_program.bindings.descriptors.push_back(
		    {ShaderRecompiler::IR::DescriptorBindingKind::Buffers, {0u}});
	...
	{
		const char* name = "no native buffers: prepare, publish, commit";
		auto& executor = context.render_context->GetRenderExecutor();
		auto program = std::make_shared<ShaderRecompiler::IR::Program>();
		program->stage = ShaderType::Compute;
		program->resource_tracking_complete = true;
		program->info.buffers.resize(2);
		auto snapshot = std::make_shared<ShaderRecompiler::IR::ResourceSnapshot>();
		snapshot->buffers.resize(2);
		for (uint32_t i = 0; i < 2; ++i) {
			ShaderBufferResource descriptor{};
			descriptor.UpdateAddress48(guest + 0x360000 + uint64_t(i) * 0x4000);
			descriptor.fields[1] |= 16u << 16u;
			descriptor.fields[2] = 16u;
			descriptor.fields[3] = DstSel(4, 5, 6, 7);
			auto& raw = snapshot->buffers[i];
			std::copy(std::begin(descriptor.fields), std::end(descriptor.fields), raw.dwords.begin());
			raw.dword_count = 4;
			program->info.buffers[i].packed_stride = descriptor.PackedStride();
			program->info.buffers[i].descriptor_format = descriptor.Format();
			program->info.buffers[i].descriptor_swizzle = descriptor.DstSelXYZW();
		}
		ShaderStageRuntime runtime{program, snapshot};
		auto prepared = executor.PrepareBindings(runtime);
		executor.FindBuffers(prepared);
		PreparedBindings* stages[1] = {&prepared};
		executor.FinalizeBindings(std::span<PreparedBindings* const>{stages, 1u});
		Check(name, prepared.buffer_ids.size() == 2 && prepared.buffer_ids[0] == BufferId {} &&
		                prepared.buffer_ids[1] == BufferId {} && prepared.resources.buffers.size() == 2 &&
		                prepared.resources.buffers[0].buffer == nullptr &&
		                prepared.resources.buffers[1].buffer == nullptr && prepared.published,
		      "no-native program acquired or published a storage-buffer binding");
		auto pipeline = Libs::Graphics::RenderExecutorTestAccess::CommitSingleStage(
		    executor, scheduler.Current(), prepared);
		scheduler.FlushAndWait();
		Libs::Graphics::RenderExecutorTestAccess::Destroy(executor, pipeline);
		if (prepared.published && prepared.resources.buffers[0].buffer == nullptr &&
		    prepared.resources.buffers[1].buffer == nullptr) {
			Pass(name, "declared snapshot slots stayed out of native acquisition and commit");
		}
	}

	// ------------------------------------------------------------------------------------
	// Sparse native buffers must remain original-indexed through the production preparation
	// path. Slots 0 and 2 deliberately contain valid, guest-mapped descriptors: if this test
	// walked every declared buffer, they would acquire native cache entries. Only slots 1 (read)
	// and 3 (writable) survive in the dense Vulkan binding, while publication keeps the original
	// slots so a range replacement can republish slot 3 correctly before CommitBindings records
	// the push descriptors.
	{
		const char* name = "sparse native buffers: prepare, publish, commit";
		auto& executor = context.render_context->GetRenderExecutor();
		const uint64_t base = guest + 0x340000;
		for (uint32_t i = 0; i < 4; ++i) {
			GuestWrite(base + uint64_t(i) * 0x4000, 0x100, static_cast<uint8_t>(0x80u + i));
		}

		auto program = std::make_shared<ShaderRecompiler::IR::Program>();
		program->stage = ShaderType::Compute;
		program->resource_tracking_complete = true;
		program->info.buffers.resize(4);
		program->bindings.memory_offset_dword = 0;
		program->bindings.memory_offset_count = 2;
		program->bindings.descriptors.push_back(
		    {ShaderRecompiler::IR::DescriptorBindingKind::Buffers, {1u, 3u}});

		auto snapshot = std::make_shared<ShaderRecompiler::IR::ResourceSnapshot>();
		snapshot->buffers.resize(4);
		for (uint32_t i = 0; i < 4; ++i) {
			ShaderBufferResource descriptor{};
			descriptor.UpdateAddress48(base + uint64_t(i) * 0x4000);
			descriptor.fields[1] |= 16u << 16u;
			descriptor.fields[2] = 16u;
			descriptor.fields[3] = DstSel(4, 5, 6, 7);
			auto& raw = snapshot->buffers[i];
		...
		program->info.buffers[1].read = true;
		program->info.buffers[3].written = true;
		ShaderStageRuntime runtime{program, snapshot};

		auto prepared = executor.PrepareBindings(runtime);
		executor.FindBuffers(prepared);
		Check(name, prepared.buffer_ids.size() == 4 && prepared.buffer_ids[0] == BufferId {} &&
		                prepared.buffer_ids[2] == BufferId {},
		      "excluded sparse slots reached native buffer acquisition");
		Check(name, prepared.buffer_ids[1] != BufferId {} && prepared.buffer_ids[3] != BufferId {},
		      "surviving read/write slots were not acquired");

		// Force a replacement after acquisition. FinalizeBindings rebinds and republishes the
		// writable surviving slot before CommitBindings emits its dense descriptor array.
		cache.ObtainBuffer(base + 3 * 0x4000 - BufferCache::CACHING_PAGESIZE,
		                   0x100 + 2 * BufferCache::CACHING_PAGESIZE, false);
		GuestWrite(base + 3 * 0x4000, 0x100, 0xD3);
		cache.ObtainBuffer(base + 3 * 0x4000, 0x100, false);
		PreparedBindings* stages[1] = {&prepared};
		executor.FinalizeBindings(std::span<PreparedBindings* const>{stages, 1u});
		Check(name, prepared.resources.buffers.size() == 4 &&
		                prepared.resources.buffers[0].buffer == nullptr &&
		                prepared.resources.buffers[2].buffer == nullptr,
		      "excluded slots acquired a native view during rebind/publication");
		Check(name, prepared.resources.buffers[1].buffer != nullptr &&
		                prepared.resources.buffers[3].buffer != nullptr && prepared.published,
		      "surviving sparse bindings were not rebound and published");
		const auto value = readback.ReadByte(context, prepared.resources.buffers[3].buffer,
		                                    prepared.resources.buffers[3].offset);
		Check(name, value == 0xD3, "writable surviving slot did not republish the replacement");
		auto& scheduler = context.render_context->GetCommandScheduler();
		auto pipeline = Libs::Graphics::RenderExecutorTestAccess::CommitSingleStage(
		    executor, scheduler.Current(), prepared);
		scheduler.FlushAndWait();
		Libs::Graphics::RenderExecutorTestAccess::Destroy(executor, pipeline);
		if (prepared.buffer_ids[0] == BufferId {} && prepared.buffer_ids[2] == BufferId {} &&
		    prepared.resources.buffers[0].buffer == nullptr && prepared.resources.buffers[2].buffer == nullptr &&
		    prepared.resources.buffers[1].buffer != nullptr && prepared.resources.buffers[3].buffer != nullptr &&
		    value == 0xD3) {
			Pass(name, "excluded slots never acquired; read/write survivors republished and committed");
		}
	}
