...
int main() {
	...
	// Shared checksum parser (Config::ParsePixelShaderChksum, also used by --skip-ps-chksum).
	{
		struct Case {
			const char* text;
			bool        ok;
		...
		for (const auto& c: cases) {
			uint64_t   v  = 0;
			const bool ok = Config::ParsePixelShaderChksum(c.text, v);
		...
		uint64_t v = 0;
		if (Config::ParsePixelShaderChksum(nullptr, v)) {
			bad++;
		}
